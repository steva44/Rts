...
RDispBeamColumn3d::RDispBeamColumn3d(QObject *parent, QString name)
...
RDispBeamColumn3d::~RDispBeamColumn3d()
{

}


int RDispBeamColumn3d::update()
{
...
int RDispBeamColumn3d::generateOpenSeesInput(std::string& object, int flag)
{
    ...
    std::string numIntgrPts   = std::to_string(numSections);

    std::string transfTag   = std::to_string(theCrdTransformation->getTag());
    std::string integType   = theIntegrationType.toStdString();
    std::string massDensity = std::to_string(theRhoParameter->getCurrentValue());

    if(flag == 1) //tcl file
    {
        //        out<<"# element dispBeamColumn $eleTag $iNode $jNode $numIntgrPts -sections $secTag1 $secTag2 ... $transfTag <-mass $massDens> <-cMass> <-integration $intType>"<<std::endl;
        out<<"element dispBeamColumn "<<tag<<" "<<iNode<<" "<<jNode<<" "<<numIntgrPts<<" -sections ";
        ...
        for (size_t i=0; i<numSections; ++i)
        {
            ...
            std::string secTag   = std::to_string(theRSection->getObjectTag());

            out<<secTag<<" ";
        }

        // Now the rest
        out<<transfTag<<" "<<massDensity<<""<<" -integration "<<integType<<std::endl;
    }
    else if(flag == 2) //Python
    {
        std::ostringstream elemList;
        elemList<<"elemList"<<tag<<" = ["<<iNode<<", "<<jNode<<" ];"<<std::endl;

        out<<"Implement me for Python in "<<__FUNCTION__<<std::endl;

        //Here we have to create an integration object and give the tag...could not find this implementation in OpenSeesPy, to be finished

        //        out<<"element('dispBeamColumn', eleTag, *eleNodes, transfTag, integrationTag, '-cMass', '-mass', mass=0.0)"<<std::endl;
        //        out<<"element('dispBeamColumn', "<<tag<<", "<<elemList.str()<<", "<<secTag<<")"<<std::endl;
    }
    else
    {
        return -1;
...
Element* RDispBeamColumn3d::getTheOpenSeesElement(void)
{
    if(!theOpenSeesElement)
        this->createDispBeamColumn3dElement();

    return theOpenSeesElement;
}


int RDispBeamColumn3d::createDispBeamColumn3dElement(void)
{
...
void RDispBeamColumn3d::print(void)
{
    qDebug()<<"***********************************************\n";
...
int RDispBeamColumn3d::initialize(void)
{
    ...
    theOpenSeesElement = new DispBeamColumn3d(objectTag, Nd1, Nd2, numSections, sections,
...
int RDispBeamColumn3d::getGlobalResponse(QVector<double> *response, QString type,int sectionNum)
{
...
int RDispBeamColumn3d::getLocalResponse(QVector<double> *response, QString type, int sectionNum)
{
...
int RDispBeamColumn3d::getLocalDisplacementResponse(QVector<double> *response, int sectionNum)
{  
...
//(*response)[10]="My_2"
//(*response)[11]="Mz_2"
int RDispBeamColumn3d::getLocalForces(QVector<double> *response)
{
    if(response->size() != 12)
    {
...
//(*response)[10] = "Vz"
//(*response)[11] = "T"
int RDispBeamColumn3d::getLocalForceResponse(QVector<double> *response, int sectionNum)
{
    ...
    //NOTE: be careful with the section numbering, the sections start from 1..n not 0..n so add +1 to compensate
...
int RDispBeamColumn3d::getLocalStressResponse(QVector<double> *response, int sectionNum)
{
...
int RDispBeamColumn3d::getLocalStrainResponse(QVector<double> *response, int sectionNum)
{
    auto res = this->getFibreResponses(response,sectionNum);
...
int RDispBeamColumn3d::getFibreResponses(QVector<double> *response, int sectionNum)
{
    if (sectionNum > numSections)
    {
        qDebug()<<"Error, the section number "<<sectionNum<<" in "<<__FUNCTION__<<" is greater than the number of sections: "<<numSections<<" in the element: "<<this->objectName();
