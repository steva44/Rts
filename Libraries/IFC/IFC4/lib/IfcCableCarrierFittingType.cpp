...
IfcCableCarrierFittingType::IfcCableCarrierFittingType( int id ) { m_entity_id = id; }
IfcCableCarrierFittingType::~IfcCableCarrierFittingType() {}
...
{
	stream << "#" << m_entity_id << "= IFCCABLECARRIERFITTINGTYPE" << "(";
...
void IfcCableCarrierFittingType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcCableCarrierFittingType::toString() const { return L"IfcCableCarrierFittingType"; }
void IfcCableCarrierFittingType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCableCarrierFittingType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCableCarrierFittingTypeEnum::createObjectFromSTEP( args[9], map );
}
