...
IfcElementComponent::IfcElementComponent( int id ) { m_entity_id = id; }
IfcElementComponent::~IfcElementComponent() {}
...
{
	stream << "#" << m_entity_id << "= IFCELEMENTCOMPONENT" << "(";
...
void IfcElementComponent::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcElementComponent::toString() const { return L"IfcElementComponent"; }
void IfcElementComponent::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 8 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElementComponent, expecting 8, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
