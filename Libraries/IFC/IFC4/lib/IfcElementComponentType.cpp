...
IfcElementComponentType::IfcElementComponentType( int id ) { m_entity_id = id; }
IfcElementComponentType::~IfcElementComponentType() {}
...
{
	stream << "#" << m_entity_id << "= IFCELEMENTCOMPONENTTYPE" << "(";
...
void IfcElementComponentType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcElementComponentType::toString() const { return L"IfcElementComponentType"; }
void IfcElementComponentType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcElementComponentType, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
