...
{
	stream << "#" << m_entity_id << "= IFCSURFACEFEATURE" << "(";
...
void IfcSurfaceFeature::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSurfaceFeature, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSurfaceFeatureTypeEnum::createObjectFromSTEP( args[8], map );
}
