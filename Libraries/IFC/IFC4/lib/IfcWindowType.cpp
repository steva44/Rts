...
{
	stream << "#" << m_entity_id << "= IFCWINDOWTYPE" << "(";
...
void IfcWindowType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	m_PredefinedType = IfcWindowTypeEnum::createObjectFromSTEP( args[9], map );
	m_PartitioningType = IfcWindowTypePartitioningEnum::createObjectFromSTEP( args[10], map );
