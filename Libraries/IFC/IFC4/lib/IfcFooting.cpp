...
IfcFooting::IfcFooting( int id ) { m_entity_id = id; }
IfcFooting::~IfcFooting() {}
...
void IfcFooting::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcFooting::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFooting, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcFootingTypeEnum::createObjectFromSTEP( args[8], map );
}
