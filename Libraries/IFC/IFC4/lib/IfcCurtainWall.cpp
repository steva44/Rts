...
IfcCurtainWall::IfcCurtainWall( int id ) { m_entity_id = id; }
IfcCurtainWall::~IfcCurtainWall() {}
...
{
	stream << "#" << m_entity_id << "= IFCCURTAINWALL" << "(";
...
void IfcCurtainWall::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcCurtainWall::toString() const { return L"IfcCurtainWall"; }
void IfcCurtainWall::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCurtainWall, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCurtainWallTypeEnum::createObjectFromSTEP( args[8], map );
}
