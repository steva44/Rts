...
IfcControllerType::IfcControllerType( int id ) { m_entity_id = id; }
IfcControllerType::~IfcControllerType() {}
...
{
	stream << "#" << m_entity_id << "= IFCCONTROLLERTYPE" << "(";
...
void IfcControllerType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcControllerType::toString() const { return L"IfcControllerType"; }
void IfcControllerType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcControllerType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcControllerTypeEnum::createObjectFromSTEP( args[9], map );
}
