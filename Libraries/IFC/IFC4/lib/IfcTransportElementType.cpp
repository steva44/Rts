...
{
	stream << "#" << m_entity_id << "= IFCTRANSPORTELEMENTTYPE" << "(";
...
void IfcTransportElementType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcTransportElementType::toString() const { return L"IfcTransportElementType"; }
void IfcTransportElementType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTransportElementType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
