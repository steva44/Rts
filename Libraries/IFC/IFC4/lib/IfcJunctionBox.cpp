...
IfcJunctionBox::IfcJunctionBox( int id ) { m_entity_id = id; }
IfcJunctionBox::~IfcJunctionBox() {}
...
{
	stream << "#" << m_entity_id << "= IFCJUNCTIONBOX" << "(";
...
void IfcJunctionBox::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcJunctionBox::toString() const { return L"IfcJunctionBox"; }
void IfcJunctionBox::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcJunctionBox, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcJunctionBoxTypeEnum::createObjectFromSTEP( args[8], map );
}
