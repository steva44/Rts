...
IfcChimneyType::IfcChimneyType( int id ) { m_entity_id = id; }
IfcChimneyType::~IfcChimneyType() {}
...
{
	stream << "#" << m_entity_id << "= IFCCHIMNEYTYPE" << "(";
...
void IfcChimneyType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcChimneyType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcChimneyType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcChimneyTypeEnum::createObjectFromSTEP( args[9], map );
}
