...
IfcCurtainWallType::IfcCurtainWallType( int id ) { m_entity_id = id; }
IfcCurtainWallType::~IfcCurtainWallType() {}
...
{
	stream << "#" << m_entity_id << "= IFCCURTAINWALLTYPE" << "(";
...
void IfcCurtainWallType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcCurtainWallType::toString() const { return L"IfcCurtainWallType"; }
void IfcCurtainWallType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcCurtainWallType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcCurtainWallTypeEnum::createObjectFromSTEP( args[9], map );
}
