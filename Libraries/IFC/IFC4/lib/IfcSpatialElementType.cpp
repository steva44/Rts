...
IfcSpatialElementType::IfcSpatialElementType( int id ) { m_entity_id = id; }
IfcSpatialElementType::~IfcSpatialElementType() {}
...
{
	stream << "#" << m_entity_id << "= IFCSPATIALELEMENTTYPE" << "(";
...
void IfcSpatialElementType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSpatialElementType::toString() const { return L"IfcSpatialElementType"; }
void IfcSpatialElementType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSpatialElementType, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
