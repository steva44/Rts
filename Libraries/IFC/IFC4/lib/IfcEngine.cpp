...
// ENTITY IfcEngine 
IfcEngine::IfcEngine() {}
IfcEngine::IfcEngine( int id ) { m_entity_id = id; }
IfcEngine::~IfcEngine() {}
...
void IfcEngine::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcEngine::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcEngine, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
