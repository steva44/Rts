...
IfcPlateStandardCase::IfcPlateStandardCase( int id ) { m_entity_id = id; }
IfcPlateStandardCase::~IfcPlateStandardCase() {}
...
{
	stream << "#" << m_entity_id << "= IFCPLATESTANDARDCASE" << "(";
...
void IfcPlateStandardCase::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcPlateStandardCase::toString() const { return L"IfcPlateStandardCase"; }
void IfcPlateStandardCase::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPlateStandardCase, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
