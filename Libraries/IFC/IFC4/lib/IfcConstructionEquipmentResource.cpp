...
IfcConstructionEquipmentResource::IfcConstructionEquipmentResource( int id ) { m_entity_id = id; }
IfcConstructionEquipmentResource::~IfcConstructionEquipmentResource() {}
...
{
	stream << "#" << m_entity_id << "= IFCCONSTRUCTIONEQUIPMENTRESOURCE" << "(";
...
void IfcConstructionEquipmentResource::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcConstructionEquipmentResource::toString() const { return L"IfcConstructionEquipmentResource"; }
void IfcConstructionEquipmentResource::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcConstructionEquipmentResource, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcConstructionEquipmentResourceTypeEnum::createObjectFromSTEP( args[10], map );
}
