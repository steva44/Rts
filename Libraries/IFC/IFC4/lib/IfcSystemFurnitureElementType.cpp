...
IfcSystemFurnitureElementType::IfcSystemFurnitureElementType( int id ) { m_entity_id = id; }
IfcSystemFurnitureElementType::~IfcSystemFurnitureElementType() {}
...
{
	stream << "#" << m_entity_id << "= IFCSYSTEMFURNITUREELEMENTTYPE" << "(";
...
void IfcSystemFurnitureElementType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSystemFurnitureElementType::toString() const { return L"IfcSystemFurnitureElementType"; }
void IfcSystemFurnitureElementType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSystemFurnitureElementType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSystemFurnitureElementTypeEnum::createObjectFromSTEP( args[9], map );
}
