...
IfcFireSuppressionTerminalType::IfcFireSuppressionTerminalType( int id ) { m_entity_id = id; }
IfcFireSuppressionTerminalType::~IfcFireSuppressionTerminalType() {}
...
{
	stream << "#" << m_entity_id << "= IFCFIRESUPPRESSIONTERMINALTYPE" << "(";
...
void IfcFireSuppressionTerminalType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcFireSuppressionTerminalType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcFireSuppressionTerminalType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
