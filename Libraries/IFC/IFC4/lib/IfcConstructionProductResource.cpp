...
IfcConstructionProductResource::IfcConstructionProductResource( int id ) { m_entity_id = id; }
IfcConstructionProductResource::~IfcConstructionProductResource() {}
...
void IfcConstructionProductResource::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcConstructionProductResource::toString() const { return L"IfcConstructionProductResource"; }
void IfcConstructionProductResource::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcConstructionProductResource, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcConstructionProductResourceTypeEnum::createObjectFromSTEP( args[10], map );
}
