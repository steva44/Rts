...
IfcConstructionMaterialResource::IfcConstructionMaterialResource( int id ) { m_entity_id = id; }
IfcConstructionMaterialResource::~IfcConstructionMaterialResource() {}
...
{
	stream << "#" << m_entity_id << "= IFCCONSTRUCTIONMATERIALRESOURCE" << "(";
...
void IfcConstructionMaterialResource::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcConstructionMaterialResource::toString() const { return L"IfcConstructionMaterialResource"; }
void IfcConstructionMaterialResource::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 11 ){ std::stringstream err; err << "Wrong parameter count for entity IfcConstructionMaterialResource, expecting 11, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcConstructionMaterialResourceTypeEnum::createObjectFromSTEP( args[10], map );
}
