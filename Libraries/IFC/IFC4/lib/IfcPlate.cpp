...
// ENTITY IfcPlate 
IfcPlate::IfcPlate() {}
IfcPlate::IfcPlate( int id ) { m_entity_id = id; }
IfcPlate::~IfcPlate() {}
...
void IfcPlate::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcPlate::toString() const { return L"IfcPlate"; }
void IfcPlate::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcPlate, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
