...
IfcIShapeProfileDef::IfcIShapeProfileDef( int id ) { m_entity_id = id; }
IfcIShapeProfileDef::~IfcIShapeProfileDef() {}
...
{
	stream << "#" << m_entity_id << "= IFCISHAPEPROFILEDEF" << "(";
...
void IfcIShapeProfileDef::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
...
void IfcIShapeProfileDef::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcIShapeProfileDef, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
