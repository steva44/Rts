...
IfcSlabType::IfcSlabType( int id ) { m_entity_id = id; }
IfcSlabType::~IfcSlabType() {}
...
{
	stream << "#" << m_entity_id << "= IFCSLABTYPE" << "(";
...
void IfcSlabType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSlabType::toString() const { return L"IfcSlabType"; }
void IfcSlabType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSlabType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
