...
IfcAudioVisualApplianceType::IfcAudioVisualApplianceType( int id ) { m_entity_id = id; }
IfcAudioVisualApplianceType::~IfcAudioVisualApplianceType() {}
...
{
	stream << "#" << m_entity_id << "= IFCAUDIOVISUALAPPLIANCETYPE" << "(";
...
void IfcAudioVisualApplianceType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcAudioVisualApplianceType::toString() const { return L"IfcAudioVisualApplianceType"; }
void IfcAudioVisualApplianceType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcAudioVisualApplianceType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcAudioVisualApplianceTypeEnum::createObjectFromSTEP( args[9], map );
}
