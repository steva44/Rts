...
{
	stream << "#" << m_entity_id << "= IFCREINFORCINGBAR" << "(";
...
void IfcReinforcingBar::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 14 ){ std::stringstream err; err << "Wrong parameter count for entity IfcReinforcingBar, expecting 14, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_NominalDiameter = IfcPositiveLengthMeasure::createObjectFromSTEP( args[9], map );
	m_CrossSectionArea = IfcAreaMeasure::createObjectFromSTEP( args[10], map );
	m_BarLength = IfcPositiveLengthMeasure::createObjectFromSTEP( args[11], map );
