...
IfcController::IfcController( int id ) { m_entity_id = id; }
IfcController::~IfcController() {}
...
{
	stream << "#" << m_entity_id << "= IFCCONTROLLER" << "(";
...
void IfcController::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcController::toString() const { return L"IfcController"; }
void IfcController::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcController, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcControllerTypeEnum::createObjectFromSTEP( args[8], map );
}
