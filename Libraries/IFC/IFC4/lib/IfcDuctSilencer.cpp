...
IfcDuctSilencer::IfcDuctSilencer( int id ) { m_entity_id = id; }
IfcDuctSilencer::~IfcDuctSilencer() {}
...
{
	stream << "#" << m_entity_id << "= IFCDUCTSILENCER" << "(";
...
void IfcDuctSilencer::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcDuctSilencer::toString() const { return L"IfcDuctSilencer"; }
void IfcDuctSilencer::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	...
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcDuctSilencer, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcDuctSilencerTypeEnum::createObjectFromSTEP( args[8], map );
}
