...
IfcLightFixture::IfcLightFixture( int id ) { m_entity_id = id; }
IfcLightFixture::~IfcLightFixture() {}
...
{
	stream << "#" << m_entity_id << "= IFCLIGHTFIXTURE" << "(";
...
void IfcLightFixture::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcLightFixture::toString() const { return L"IfcLightFixture"; }
void IfcLightFixture::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcLightFixture, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcLightFixtureTypeEnum::createObjectFromSTEP( args[8], map );
}
