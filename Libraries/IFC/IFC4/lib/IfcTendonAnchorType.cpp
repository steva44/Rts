...
IfcTendonAnchorType::IfcTendonAnchorType( int id ) { m_entity_id = id; }
IfcTendonAnchorType::~IfcTendonAnchorType() {}
...
{
	stream << "#" << m_entity_id << "= IFCTENDONANCHORTYPE" << "(";
...
void IfcTendonAnchorType::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcTendonAnchorType::toString() const { return L"IfcTendonAnchorType"; }
void IfcTendonAnchorType::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 10 ){ std::stringstream err; err << "Wrong parameter count for entity IfcTendonAnchorType, expecting 10, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
