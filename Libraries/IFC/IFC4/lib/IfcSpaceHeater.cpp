...
IfcSpaceHeater::IfcSpaceHeater( int id ) { m_entity_id = id; }
IfcSpaceHeater::~IfcSpaceHeater() {}
...
{
	stream << "#" << m_entity_id << "= IFCSPACEHEATER" << "(";
...
void IfcSpaceHeater::getStepParameter( std::stringstream& stream, bool ) const { stream << "#" << m_entity_id; }
const std::wstring IfcSpaceHeater::toString() const { return L"IfcSpaceHeater"; }
void IfcSpaceHeater::readStepArguments( const std::vector<std::wstring>& args, const std::map<int,shared_ptr<IfcPPEntity> >& map )
{
	const size_t num_args = args.size();
	if( num_args != 9 ){ std::stringstream err; err << "Wrong parameter count for entity IfcSpaceHeater, expecting 9, having " << num_args << ". Entity ID: " << m_entity_id << std::endl; throw IfcPPException( err.str().c_str() ); }
	...
	m_PredefinedType = IfcSpaceHeaterTypeEnum::createObjectFromSTEP( args[8], map );
}
